-- 012_add_produto_esgotado.sql
-- Adiciona o flag "esgotado" (sem estoque hoje) aos produtos.
-- Diferente de `disponivel = false` (que SOME do cardápio), um produto esgotado
-- continua aparecendo no cardápio, mas é exibido riscado e NÃO pode ser pedido.
-- A RPC criar_item_pedido passa a recusar produto esgotado (defesa no servidor).

alter table public.produtos
  add column if not exists esgotado boolean not null default false;

-- Recria a RPC com a guarda de "esgotado" (idempotente: CREATE OR REPLACE).
